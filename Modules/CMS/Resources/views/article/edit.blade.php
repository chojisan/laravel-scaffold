      ...
      <div class="col">
          ...
          <div class="card">
            ...
            <div class="card-header">
              <h3 class="mb-0">{{ __('Update Article') }}</h3>
            </div>
            <!-- Card body -->
            <div class="card-body">
                @if($errors)
...
@endif
              <form method="POST" action="{{ route('articles.update', $article->id) }}" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                      <input class="form-control" type="text" id="slug" name="slug" value="{{ $article->slug }}">
                    </div>
                  </div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <label class="custom-toggle custom-toggle-success">
                            <input type="checkbox" name="featured" id="featured" value="1" @if($article->featured == 1) checked @endif>
                    ...
                    <div class="col-md-10">
                        <select class="form-control" id="category_id" name="category_id">
                            @forelse($categories as $cat)
                                <option value="{{ $cat->id }}" @if($cat->id == $article->category_id) selected @endif>{{ $cat->name }}</option>
                                @empty
                            @endforelse
                        </select>
                    </div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-md-10">
                      <textarea class="form-control" id="short_description" name="short_description" rows="3" resize="none">{{ $article->short_description }}</textarea>
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="description" class="col-md-2 col-form-label form-control-label">Description</label>
                    <div class="col-md-10">
                        <textarea class="form-control" id="short_description" name="description" rows="3" resize="none">{{ $article->description }}</textarea>
                    </div>
                  </div>
                  <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <select multiple class="form-control" id="tags" name="tags[]">
                            @forelse($tags as $tag)
                                <option value="{{ $tag->id }}" @if($article->tags->contains($tag->id)) selected @endif>{{ $tag->name }}</option>
                                @empty
                            @endforelse
                        </select>
                    </div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        ...
                        <input type="file" accept="image/*" class="custom-file-input" id="cover_image" name="cover_image" value="{{ $article->cover_image }}">
                    ...
                    <div class="col-md-10">
                        <select class="form-control" id="order" name="order">
                            @for($i = 1; $i <= $article_count; $i++)
                                <option @if($i == $article->order) selected @endif>{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
                  </div>
